...
const subsidy=50
//数据库文件名称
const dbFile="blockTransactionDBV2"
//传世区块的内容
const genesisCoinBaseData = "The Times 03/Jan/2009 Chancellor on brink of second bailout for banks"
//地址产生算法的版本
const version = byte(0x00)
const addressChecksumLen = 4
const walletFile = "walletV2.dat"
...
func main() {
	...
	//创建创世块
	//createBlockchain("1M9LNDuZsRQcky45Cx3MtPaftyAWWWQ61E")

	//币值转移
	//send("1M9LNDuZsRQcky45Cx3MtPaftyAWWWQ61E","17u9y7sCu2taVrmrfbruNdDk3KnZAByrGu",10)
	...
	//获取钱包地址的帐户余额
	//getBalance("1M9LNDuZsRQcky45Cx3MtPaftyAWWWQ61E")
...
//寻找含有未消费输出的交易记录(废弃了这个方法是找到自己未消费的输出记录，缺点是每次都要扫描的整个区块链，现在已经将未消费的格外保存在另外一个链中了)
...
func (tx *Transaction) Verify(prevTXs map[string]Transaction) bool {
	...
	for inID, vin := range tx.Vin {
		...
		prevTx := prevTXs[hex.EncodeToString(vin.Txid)]
		//新增交易找到对应的上一个交易记录（主要是为了拿到对应的上一个交易输出的公钥哈希值），重新生成一个副本（按照签名时的副本流程再走一遍），将副本进行哈希计算（即交易哈希ID）。然后在将新增中交易输入的签名用交易输入的公钥去进行非对称解码。最后将解码的值和副本哈希计算的值进行比较，检查是否一致
		txCopy.Vin[inID].Signature = nil
		//主要是为了拿到对应的上一个交易输出的公钥哈希值，按照签名时的副本流程再走一遍
		txCopy.Vin[inID].PubKey = prevTx.Vout[vin.Vout].PubKeyHash
		//即交易哈希ID(交易签名的时候，其实最终就是对交易数据哈希计算后的交易ID进行签名)
		txCopy.ID = txCopy.Hash()
		txCopy.Vin[inID].PubKey = nil
		r := big.Int{}
		s := big.Int{}
